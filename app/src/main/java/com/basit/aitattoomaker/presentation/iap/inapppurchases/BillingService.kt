...
//                    FirebaseEvents.firebaseUserAction("IAPP", "confirm_purchased_fail")
//                    FirebaseEvents.firebaseUserAction("IAPP", "purchased_not_acknowledged")
            ...
            if (billingResult.isOk()) {
                ...
                productDetails.mapNotNull { entry ->
                    ...
                    entry.value?.let {
                        when(it.productType){
                                BillingClient.ProductType.SUBS -> {
                                    val discount = it.subscriptionOfferDetails
                                        ?.flatMap { offer -> offer.offerTags.orEmpty() }
                                        ?.firstOrNull { tag -> tag.startsWith("off-") }
                                        ?.substringAfter("off-")
                                        ?.toIntOrNull()

                                    val allPhases = it.subscriptionOfferDetails
                                        ?.getOrNull(0)
                                        ?.pricingPhases
                                        ?.pricingPhaseList
                                        ?.map { pricingPhase ->
                                            DataWrappers.ProductDetails(
                                                ...
                                                freeTrail = pricingPhase.formattedPrice,
                                                offerPercentage = null // Regular pricing has no offer
                                            )
                                        } ?: listOf()

                                    val filteredOffer = if (it.title == "Monthly Subscription Offer") {
                                    ...
                                    entry.key to (filteredOffer.ifEmpty { allPhases })
                                }

                            else->{
//                                Log.d("checkSubscription","Subs OfferTag: ${it.subscriptionOfferDetails?.getOrNull(0)?.offerTags}")
//                                Log.d("checkSubscription","Subs basePlanID: ${it.subscriptionOfferDetails?.getOrNull(0)?.basePlanId}")
                                entry.key to listOf(
                                    DataWrappers.ProductDetails(
                                        title = it.title,
                                        description = it.description,
